...
Эти псевдоклассы используются для стилизации полей ввода, которые поддерживают атрибуты `min`/`max`.

- `:in-range` для полей, в которых введённое значение **попадает внутрь** диапазона `[min, max]`;
- `:out-of-range` для полей, в которых введённое значение **не попадает внутрь** диапазона `[min, max]`;
